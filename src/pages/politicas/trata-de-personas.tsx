...
export default function TrataDePersonas() {
  const { data } = useContentful({ id: FooterID });

  if (!data || !data.fields) return null;

  const field = data.fields.contactTrataPersonas;
  ...
  return (
    <>
      <Head>
        <title>Trata de Personas — Volar</title>
        <meta name="robots" content="noindex" />
      </Head>
      <div style={{ width: "100%", height: "calc(100vh - 80px)", display: "flex", flexDirection: "column" }}>
        {viewerUrl ? (
          <iframe src={viewerUrl} style={{ flex: 1, border: "none", width: "100%", height: "100%" }} title="Trata de Personas" />
        ) : (
