...
const ComunidadesID = "3Dazc95S7xWCDX6zot2gnd";

interface ImageProps {
  id?: string;
}

export const Proyecto3 = ({ id }: ImageProps) => {
    const { data } = useContentful({ id: ComunidadesID });

    const floatingShapes = Array.from({ length: 30 }).map((_, index) => {
    const isLargeShape = Math.random() > 0.5;
    const size = isLargeShape ? 90 : 79;
    const duration = 13 + Math.random() * 15;
    const delay = Math.random() * 3;
    const left = `${Math.random() * 100}%`;
    const bottom = `${-size - Math.random() * 100}px`;
    return (
    <div
        key={index}
        ...
        {proSteps.map ((item: any, index:number)=> {
            ...
            <section id={id} style={{width:'100%', overflow:'hidden', backgroundColor: proColor, padding:'2em 0 3em 0'}}>
              <Swiper
                ...
                loop={true}
                spaceBetween={20}
                style={{ width: '85%', margin: '2em auto 0' }}
                breakpoints={{
                  0: { slidesPerView: 1 },
                  790: { slidesPerView: 2 },
